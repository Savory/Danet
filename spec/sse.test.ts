    ...
    sendUpdate(): EventTarget {
        ...
        const interval = setInterval(() => {
            if (id >= 4) {
                ...
                const event = new SSEEvent({ retry: 1000, id: `${id}`, data: 'close', event: 'close' });
                eventTarget.dispatchEvent(event)
                return;
            }
            const event =  new SSEEvent({ retry: 1000, id: `${id}`, data: 'world', event: 'hello' });
            eventTarget.dispatchEvent(event)
            id++;
        }, 100);
        return eventTarget;
...
Deno.test('Body', async () => {
	return new Promise<void>(async (resolve, reject) => {
        ...
        let eventReceived = 0;
        const eventSource = new EventSource(`http://localhost:${listenEvent.port}/sse`);
