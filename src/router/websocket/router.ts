...
import { ControllerConstructor, ExecutionContext, MiddlewareExecutor } from '../mod.ts';
...
export class WebSocketRouter {
    ...
    public registerController(Controller: Constructor, endpoint: string) {
		...
		this.router.get(path, this.handleConnectionRequest(topicRouter, Controller));
	}

    private handleConnectionRequest(topicRouter: SmartRouter<unknown>, Controller: Constructor) {
        return async (context: HttpContext) => {
            ...
            socket.onmessage = this.onMessage(topicRouter, Controller, controllerInstance, socket);
            return response;
        }
    }

    private onConnection(executionContext: ExecutionContext, Controller: Constructor, socket: WebSocket) {
            return async () => {
                try {
    ...
    private onMessage(topicRouter: SmartRouter<unknown>, Controller: Constructor, controllerInstance: any, socket: WebSocket) {
    ...
    private registerTopic(methods: string[], Controller: Constructor, topicRouter: SmartRouter<unknown>) {
